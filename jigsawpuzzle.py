...
def harmonic_color(img):
    "Returns a random color from image"
    width, height = img.get_size()
    col = img.get_at((randrange(0, width // 2), randrange(0, height // 2)))
    return col
...
def floor(number_to_round):
    ...
    str_number = ["1"]
    for _ in range(1, length_string):
        str_number.append("0")
...
class Tile:
	...
	@staticmethod
	def setValues():
		Tile.width = Puzzle.width // 10
		Tile.height = Puzzle.height // 10
